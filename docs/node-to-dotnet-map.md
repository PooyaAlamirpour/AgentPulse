...
- Node Optionهای بسیار بیشتری دارد؛ Prompt از Argument/stdin پوشش داده شده و گزینه‌های محدود `--dir`، `--model` و `--session` برای فاز ۸ باقی مانده‌اند.
