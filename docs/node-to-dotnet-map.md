...
- ماتریس کامل Baseline با وضعیت‌های Matched، Fixed، Intentionally Different و Not Applicable
- نگاشت مرکزی Exit Codeهای `0`، `1`، `2`، `3`، `4`، `5`، `124` و `130`
- قرارداد pipe-safe: پاسخ Stream روی stdout و Session ID، Error و Log روی stderr
- تشخیص تست‌پذیر Interactive/Non-Interactive و جلوگیری از Credential prompt در Pipe/CI
- تست Process واقعی برای stdin، مسیر Space، Directory/Session/Configuration نامعتبر، Provider failure و Timeout
- حفظ Partial response در Failure و Ctrl+C، Release Lease و ادامه Session
- Crash Recovery با قطع اجباری Process پس از checkpoint و اجرای موفق بعدی
- Logging استاندارد .NET، تنظیم‌پذیر، بدون ANSI و همراه Secret redaction
- مستندات `cli-compatibility.md` و `local-cli.md`
...
Logging استاندارد .NET با Configuration موجود فعال است، همه Console logها روی stderr می‌روند و فرمت redirected بدون ANSI است. تست Marker ثابت می‌کند Credential، Prompt و raw provider body در stdout، stderr یا failure metadata نشت نمی‌کنند. Telemetry/Control Plane خارج از Scope باقی مانده است.
...
- تثبیت Exit Code و stdout/stderr در مرز Process
- تست عدم نشت Secret در Log و Persistence metadata
...
- Node Optionهای بسیار بیشتری دارد؛ نسخه .NET فقط `--dir`، `--model` و `--session` را همراه Prompt positional یا redirected `stdin` پیاده‌سازی می‌کند.
- `--continue`، `--format json` و JSON Event Format عمداً خارج از Scope‌اند.
- Positional prompt در .NET بر redirected stdin اولویت دارد؛ Node می‌تواند stdin را به message اضافه کند.
- Session ID در .NET فقط روی stderr چاپ می‌شود و stdout فقط پاسخ مدل است.
- .NET از Lease پایدار SQLite و recovery بر پایه Expiration استفاده می‌کند.
- نگاشت نهایی اختلاف‌ها و Test نام هر رفتار در `docs/cli-compatibility.md` ثبت شده است.
